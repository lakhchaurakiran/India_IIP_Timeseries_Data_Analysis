...
Let's plot the Primary.goods data 

```{r}
plot(df$Primary.goods~df$MonYear,type="l")
```
...
Now let's look at the differenced data
...
The data looks almost stationary with no clear trend, seasonality or change in variation. 
...
We see that the correlations have reduced significantly. In the ACF plot there is significant correlation only at lag=12 which might be due to seasonality and in PACF plot also al the correlations are really small.

### Finding best orders for ARIMA model fitting using
...
### Seasonal ARIMA(0,1,1,0,1,1,12) fitting results
